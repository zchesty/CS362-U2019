...
void baronGainEstate(struct gameState *state, int currentPlayer) {
...
void minionDiscardHand(struct gameState *state, int player, int handPos) {
    int j;
     //discard hand
...
void minionHandler(struct gameState *state, int choice1, int choice2, int currentPlayer, int handPos) {
	...
	{
	  minionDiscardHand(state, currentPlayer, handPos);
...
void moveDeckToDiscard(struct gameState *state, int nextPlayer) {
    int i;
...
void tributeHandler(struct gameState *state, int currentPlayer, int nextPlayer) {
  ...
  int tributeRevealedCards[2] = {-1, -1};
    ...
    if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1){
        ...
        if (state->deckCount[nextPlayer] == 0){
            moveDeckToDiscard(state, nextPlayer);
            shuffle(nextPlayer,state);//Shuffle the deck
        }
        assignTributeCard(state, tributeRevealedCards, 0, nextPlayer);
        assignTributeCard(state, tributeRevealedCards, 1, nextPlayer);
    }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case mine:
        mineReturnValue = mineHandler(state, choice1, choice2, currentPlayer);
        return mineReturnValue;
    ...
    case ambassador:
        ambassadorReturnValue = ambassadorHandler(state, choice1, choice2, currentPlayer, handPos);
        return ambassadorReturnValue;
